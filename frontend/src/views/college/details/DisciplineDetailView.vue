      ...
      <template v-if="currentTab === 'overview'">
        ...
        <section id="grade-history" class="resource-section" :class="{ 'resource-section--active': activeSection === 'grade-history' }">
          ...
          <div class="grade-history">
            <article v-for="item in data.gradeHistory" :key="item.major" class="grade-history__card">
              <h3>{{ item.major }}</h3>
              <div class="grade-history__track">
                <div v-for="(grade, idx) in item.grades" :key="`${item.major}-${item.years[idx]}`" class="grade-history__node">
                  <strong>{{ grade }}</strong>
...
.grade-history {
  ...
  &__node {
    ...
    strong { font-size: 20px; color: #ffd56a; }
    span { font-size: 14px; color: #8eaec8; }
  }
}
