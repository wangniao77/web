...
import {
  groupSummary,
  type KeyPlanGroup,
  type KeyPlanProgressData,
  type PlanGroupCategory,
  type PlanProgressStatus,
} from '@/mock/college/key-plan-progress'

const props = defineProps<{
  data: KeyPlanProgressData
}>()

const router = useRouter()

const summary = computed(() => {
  const { total, completed, ongoing, attention } = props.data.overview
  return {
    total,
    completed,
    ongoing,
...
watch(
  ...
  (groups) => {
    for (const g of groups) {
      if (!(g.id in expanded)) {
        expanded[g.id] = g.defaultExpanded === true
      }
    }
  },
...
function groupIcon(category: PlanGroupCategory): IconKind {
  ...
  if (category === 'teaching' || category === 'talent') return 'students'
  if (category === 'research') return 'research'
  if (category === 'party') return 'award'
  if (category === 'ai') return 'innovation'
  return 'academic'
}
