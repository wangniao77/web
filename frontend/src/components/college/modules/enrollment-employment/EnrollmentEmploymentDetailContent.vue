...
// 词云数据（技能 / 就业行业），权重决定字号。真实数据可接 data.graduation.skillCloud / industryCloud
type CloudWord = { name: string; weight: number }
const skillCloud: CloudWord[] = [
  { name: '编程能力', weight: 95 },
  { name: '数据分析', weight: 88 },
  { name: '团队协作', weight: 80 },
  { name: '沟通表达', weight: 72 },
  ...
  { name: '领导力', weight: 42 },
  { name: 'SQL', weight: 40 },
  { name: 'Python', weight: 38 },
  { name: 'Office', weight: 34 },
  { name: '抗压能力', weight: 30 },
  { name: '创新思维', weight: 26 },
]
const industryCloud: CloudWord[] = [
  { name: '信息技术', weight: 92 },
  { name: '金融', weight: 80 },
  { name: '教育', weight: 72 },
  { name: '智能制造', weight: 66 },
...
const cloudTab = ref<CloudTab>('skill')
const activeCloud = computed<CloudWord[]>(() =>
  cloudTab.value === 'skill' ? skillCloud : industryCloud,
)
...
const sourceMapOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    visualMap: {
      ...
      calculable: true,
      text: ['生源多', '生源少'],
      inRange: { color: ['#0b2a3a', '#0e7490', '#0891b2', '#22d3ee', '#a5f3ff'] },
    ...
    series: [{
      ...
      layoutSize: '100%',
      itemStyle: {
        borderColor: 'rgba(120, 200, 230, 0.45)',
        borderWidth: 0.6,
        areaColor: '#0a1c2b',
      },
      emphasis: {
        label: { show: true, color: '#eaf7ff', fontWeight: 'bold' },
        itemStyle: { areaColor: '#22d3ee', shadowBlur: 12, shadowColor: 'rgba(34, 211, 238, 0.7)' },
      },
      label: { show: false },
      data: provinces,
    }],
...
const distOption = computed(() => {
  ...
  return {
    grid: { left: 8, right: 88, top: 4, bottom: 4, outerBoundsMode: 'same', outerBoundsContain: 'axisLabel' },
    tooltip: {
      trigger: 'axis',
      axisPointer: { type: 'shadow' },
      formatter: (params: unknown) => {
      ...
      <div class="ee-detail__grid ee-detail__grid--admission">
        <div class="ee-detail__top-row">
          <section class="ee-detail__card ee-detail__card--map">
            <h3>生源省份分布</h3>
            <div class="ee-detail__chart ee-detail__chart--map">
        ...
        <section class="ee-detail__card" :class="{ 'is-focus': highlight === 'exit-quality' }">
          ...
          <div class="ee-detail__wordcloud">
            <div class="ee-detail__subtabs">
              <button type="button" :class="{ 'is-active': cloudTab === 'skill' }" @click="cloudTab = 'skill'">技能</button>
              <button type="button" :class="{ 'is-active': cloudTab === 'industry' }" @click="cloudTab = 'industry'">就业行业</button>
            </div>
            <div class="ee-detail__cloud">
              <WordCloud :words="activeCloud" :colors="CLOUD_COLORS" />
            </div>
          </div>
        </section>
