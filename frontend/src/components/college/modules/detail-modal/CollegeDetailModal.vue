...
const titleMap: Record<string, { icon: IconKind; title: string }> = {
  'key-tasks': { icon: 'task', title: '学院重点工作动态监测总览' },
...
const filteredPlanTasks = computed(() => {
  ...
  return keyTasks.value.tasks.filter((t) => {
    ...
    if (planFilterDomain.value !== '全部') {
      const matchLabel =
        t.categoryLabel === planFilterDomain.value || t.groupTitle === planFilterDomain.value
      const matchCategory =
        (planFilterDomain.value === '学科建设' && t.category === 'discipline') ||
        (planFilterDomain.value === '师资队伍建设' && t.category === 'faculty')
...
const groupedPlanTasks = computed(() => {
  const order = [
  ...
  return order
    .map((g) => ({
      ...g,
      tasks: filteredPlanTasks.value.filter(
          ...
          <div class="cdm-body">
            ...
            <template v-else-if="state.kind === 'key-tasks' && keyTasks">
              ...
              <div class="cdm-section">
                ...
                <div
                  v-for="group in groupedPlanTasks"
                  :key="`cdm-${group.id}`"
                  class="cdm-task-group"
                >
                  <header class="cdm-task-group__head">
                    <div>
                  ...
                  <div class="cdm-cards">
                    <div v-for="t in group.tasks" :key="t.id" class="cdm-task">
                      <div class="cdm-task__top">
                        <strong>
